...
for digital image correlation"""
     ...
     def plot_field(self, field, title):
          ...
          if not hasattr(self, '_plot_refs'):
               self._plot_refs = []
          self._plot_refs.append(Plot(image_ref, self, field, title))
...
class Plot:
    def __init__(self, image, grid, data, title):
        ...
        self.cax = self.cb.ax  # colorbar axes를 고정으로 저장
    ...
    def update(self, val):
        ...
        # 저장된 colorbar axes에 제자리에서 재렌더링 (cb.remove() 없이)
        self.cax.cla()
